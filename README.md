...
- `template/` — **锁定**的 `family-ai-chat-shell`（Cloudflare Worker + D1 + Grok 风格 UI；首屏为 Bot 身份与 4 位口令）

部署时必须**原样拷贝** `template/`，禁止用模型重写 `public/` 页面。分享 Skill 会在运行时读取所选 Bot 的名称与头像；头像获取失败时使用内置三角头像。
